<!--
  ...
  Component:    Dataset info
  Description:  Dataset info section
  ...
  <div class="text-right text-body-4 font-weight-medium pb-3 compact-text-3 success--text">
    ...
    <div v-if="lastUpdate">
      <v-tooltip bottom right allow-overflow z-index="999" :close-delay="0" max-width="400px">
        ...
        <!-- Tooltip activator: summary text -->
    ...
    <!-- Loading element -->
    <div v-else>
      <v-progress-circular class="mr-2" indeterminate size="15"/>
...
export default {
  ...
  computed: {
    ...
    /** Flag set to true if there is no lower bound on time */
    noBegin() {
      return this.datasetInfo.beginDate === 'beginning'
    },

    /** Flag set to true if there is no upper bound on time */
    noEnd() {
      return this.datasetInfo.endDate === 'end'
    },

    /** Flag set to true if there is no country filter */
    noCountryFilter() {
